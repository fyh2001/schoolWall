 ...
 * @LastEditTime: 2023-05-30 04:25:41
 * @FilePath: /schoolWall/src/views/post/components/DeTabs.vue
 * @Description: 
-->
<template>
    <div>
  ...
  const checkBox = computed(() => {
    return {
      width: checkBoxSize.value.width + 2 + "px",
      height: checkBoxSize.value.height - 5 + "px",
      left: checkBoxSize.value.left - checkBoxSize.value.tabX - 1.6 + "px",
    };
  });
  ...
  <style scoped>
  .tabs {
    @apply relative flex justify-between items-center bg-opacity-65 backdrop-blur-30 rounded-xl px-0.8 py-0.2;
    background-color: #f2f1f6;
  }
  .checkBox {
    @apply absolute bg-white rounded-xl duration-300;
  ...
  .tabs-item {
    /* @apply flex-1 */
    @apply w-full py-1 text-gray-5 text-3 text-center z-1 rounded-lg duration-300;
  }
