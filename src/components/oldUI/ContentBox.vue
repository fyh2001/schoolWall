 ...
 * @LastEditTime: 2023-05-18 23:06:14
    ...
    >
      <div class="flex justify-between mb-2">
        <div class="flex items-center">
          <div class="text-gray-5 mr-1.5">
            # {{ typeOfDisplay == "post" ? data.id : index + 1 }} ·
          </div>
      ...
      <div ref="imgBoxRef" class="mb-3">
        <div
          v-html="data.text"
          class="break-normal text-gray-8 text-4.5"
          style="text-indent: 2em"
        />
        <div v-if="data.imageUrl != null" class="flex justify-between mt-5">
          <n-image-group show-toolbar-tooltip>
            <n-space ref="spaceRef">
              <n-image
                class="rounded-md"
                lazy
                object-fit="cover"
                v-for="(image, index) in data.imageUrl"
                :style="imageShowRule(data.imageUrl.length)"
      ...
      <div class="flex justify-between">
        ...
        <div class="flex items-center" v-if="typeOfDisplay == 'post'">
          <icon-message class="mr-1" style="font-size: 16; color: #6b7280" />
...
/**
 * 图片显示规则
 * @param {*} listLength 图片数组长度
 */
const imgBoxRef = ref(null);

// const imageShowRule = computed((listLength) => {
//   switch(listLength){

//   }
// })
const imageShowRule = (listLength) => {
  if (listLength == 1) {
    return {
      width: "100%",
      height: "100%",
...
onMounted(() => {
  // console.log(imgBoxRef.value[0].clientWidth);
});
</script>
